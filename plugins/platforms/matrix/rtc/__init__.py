...
``outbound`` (the gateway's streaming-TTS contract). Only ``focus`` and ``segmenter``
are re-exported here, because they are the two that import nothing heavier than the
stdlib: reach for ``receiver``/``publisher`` only where the LiveKit SDK is expected,
and ``session``/``outbound`` only where the gateway is loaded.
"""
